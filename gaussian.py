...
,vin # v from previous level
	...
	for i in range (2,int(len(I1[:,0])-2)):
	   for j in range (2,int(len(I2[0,:])-2)):
...
def runproglk (I1,I2,iternum, nlev):
	...
	ue = DoHigh (u,G)
	ve = DoHigh (v,G)
	I1current = p1[0:int(len(p1[:,0])/(2**(nlev-k-1))),0:int(len(p1[0,:])/(2**(nlev-k-1))),nlev-k-1]
	I2current = p2[0:int(len(p2[:,0])/(2**(nlev-k-1))),0:int(len(p2[0,:])/(2**(nlev-k-1))),nlev-k-1]
...
'''
(ul01,vl01,Il01,ul11,vl11,Il11,ul21,vl21,Il21) = runproglk('1.png','2.png',3,3)
