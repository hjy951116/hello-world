...
,vin # v from previous level
	...
	for i in range (2,len(I1[:,0])-2):
	   for j in range (2,len(I2[0,:])-2):
		...
		lri = int((i-2)+vin[i,j]) #Low Row Index of the selected window
		hri = int((i+2)+vin[i,j]) #High Row Index of the selected window
		lci = int((j-2)+uin[i,j]) #Low Column Index of the selected window
		hci = int((j+2)+uin[i,j]) #High Column Index of the selected window
...
'''
(ul01,vl01,Il01,ul11,vl11,Il11,ul21,vl21,Il21) = runproglk('./Optical-Flow-master/basketball1.png','./Optical-Flow-master/basketball1.png',3,3)
